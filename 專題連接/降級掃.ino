...
void getExternalData() {
  ...
  if (client.connect(externalApiHost, externalport)) {
    ...
    if (!doc.containsKey("products")) {
      Serial.println("Missing 'products' key in JSON");
      flashOnce(redColor);
      client.stop();
      return;
    }
    ...
    // 檢查是否為空
    if (products.size() == 0) {
      Serial.println("Empty 'products' array");
      flashOnce(redColor);
      client.stop();
      return;
    }
...
void sendExerciseData() {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    switch (currentMode) {
      case PUSH_UP:
        postData["actionType"] = "Push Up";
        postData["frequency"] = count_push_up;
        break;
      case SIT_UP:
        postData["actionType"] = "Sit Up";
        postData["frequency"] = count_sit_up;
        break;
      case SQUAT:
        postData["actionType"] = "Squat";
        postData["frequency"] = count_squat;
        break;
    }
    ...
    // 發送 POST 請求
    client.post(severendpoint, "application/json", jsonString);
